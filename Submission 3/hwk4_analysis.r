...
pacman::p_load(tidyverse, ggplot2, dplyr, lubridate, stringr, readxl, data.table, gdata, rddensity, rdrobust, cobalt)
...
# Create density plots for the scores around the threshold of 2.75 stars (2.5 to 3)
dens3 <- density(ma.rd3$score, cut = 0)
plot(dens3, main = "Density Plot for Scores Around the Threshold of 3 Stars")

# Create density plots for the scores around the threshold of 3.25 stars (3 to 3.5)
...
# You want to investigate differences across plans. If you try to limit to only HMO plans, then of course there will be no difference in that dimension. 


#The threshold is 3.25 if you are looking at 3 v 3.5. Are these plans similar to eachother above and below?
#The threshold is 2.75 if you are looking at 2.5 v 3.

# Create lp.vars (Threshold is 2.75 and bandwidth is 0.125)
lp_vars1 <- ma.data.clean %>% 
  ungroup() %>%
...
# Create plot.30
plot.30 <- love.plot(bal.tab(lp_covs1, treat = lp_vars1$rounded), 
                     colors = "black", 
                     shapes = "circle") +
  theme_bw() + 
...
# Create plot.35
plot.35 <- love.plot(bal.tab(lp_covs2, treat = lp_vars2$rounded), 
                     colors = "black", 
                     shapes = "circle") +
  theme_bw() + 
  theme(legend.position = "none") +
